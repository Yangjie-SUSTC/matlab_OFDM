...
if nargout == 0,
  ...
  mag = abs(hh);   phase = angle(hh)*180/pi;
    h= figure('name','Filter amplitude response');
  loglog(w*2*pi,mag)
  ...
  ax = gca;
  saveas(h,[pwd,'\fig\block4\Amplitude response.fig'])
  saveas(h,[pwd,'\fig\detail\Amplitude response.fig'])
  
  
  h2= figure('name','Filter phase response');
  semilogx(w,phase),set(gca,'xgrid','on','ygrid','on')
  set(gca,'xlim',[w(1) w(length(w))])
   xlabel('Frequency  / MHz','fontsize',12)
  ylabel(getString(message('signal:freqs:Phasedegrees')))
  title('Phase response','fontsize',16)
  axes(ax) 
  saveas(h2,[pwd,'\fig\block4\Phase response.fig'])
  saveas(h2,[pwd,'\fig\detail\Phase response.fig'])
  close(h)
  close(h2)
elseif nargout == 1,
  h = hh;
